...
function _parseModbusBuffer(requestBuffer, vector) {
    ...
    switch (parseInt(functionCode)) {
        case 1:
        case 2:
            responseBuffer = _handleReadCoilsOrInputDiscretes(requestBuffer, vector, unitID, parseInt(functionCode));
            break;
        case 3:
 ...
 * @private
 */
function _handleReadCoilsOrInputDiscretes(requestBuffer, vector, unitID, functionCode) {
    ...
    if (functionCode == 1) {
      ...
      if (vector.getDiscreteInput) {
          for (var i = 0; i < length; i++) {
              responseBuffer.writeBit(vector.getDiscreteInput(address + i, unitID), i % 8, 3 + parseInt(i / 8));
          }
      }
    }
...
var ServerTCP = function(vector, options) {
    ...
    modbus._server.on("connection", function(sock) {
        ...
        modbus.emit("socketConnect", sock); // hkim FIX
        ...
        sock.on("error", function(err) {
            ...
            modbus.emit("socketError", sock); // hkim
        });
    });
